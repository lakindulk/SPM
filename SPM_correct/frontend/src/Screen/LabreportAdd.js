...
class LabreportAdd extends Component {
    ...
    componentDidMount() {
        ...
        axios.get('http://localhost:6500/labreport/')
            .then(response => {
    ...
    formValidation = () => {
        const { testId, chemistName, docName, date, testName, patientName, patientEmail, description } = this.state;
        let isValid = true;
        const errors = {};
        if (!testId.trim()) {
    ...
    onSubmit(e) {
        ...
        if (isValid) {
            ...
            axios.post('http://localhost:6500/labreport/create', labreport)
                .then(response => {
