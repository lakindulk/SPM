...
export class EditLabReport extends Component {
    ...
    componentDidMount() {
        ...
        axios.get(`http://localhost:6500/labreport/get/${this.props.match.params.id}`)
            .then((response) => {
    ...
    formValidation = () => {
        const { docName, date, testName, patientName, patientEmail, description } = this.state;
        let isValid = true;
        const errors = {};
        ...
        if (!date.trim()) {
            errors.dateRequire = "Date required";
            isValid = false;
        }
        if (!testName.trim()) {
    ...
    onSubmit(e) {
        ...
        if (isValid) {
            ...
            axios.put(`http://localhost:6500/labreport/edit/${id}`, labreport)
                .then(response => {
                    ...
                    <div className="row row-cols-1 row-cols-md-3 g-4">
                        ...
                        <div className="col">
                            <Link to="/labchemist/labreports">
                                <Button className="btn" variant="secondary">Previous Lab Reports</Button>
                            </Link>
                        </div>
                        <Link to="/">
                            <div className="col">
                    ...
                    <Form onSubmit={this.onSubmit} >
                        ...
                        <Row className="mb-3">
                            <Form.Group as={Col} controlId="formGridDate">
                                ...
                                />
                                {errors.dateRequire && <p style={{ color: "#C92C2E", fontSize: "17px" }}>{errors.dateRequire}</p>}
                            </Form.Group>
                        ...
                        <div className="mb-3">
                            <button type="submit" className="repbtn">Edit Report Details</button>
